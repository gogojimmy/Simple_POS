...
public class StockPanel extends JPanel {
	 ...
	 * Create the panel.
	 */
	public StockPanel(User user, Properties locale) {
		this.user = user;
		this.locale = locale;
		setLayout(null);
	...
	private void excelOutputActionPerformed(ActionEvent e) {
		...
		try {
			...
			for (Product product : list) {
				...
				label = new Label(5, i, stor);
				sheet.addCell(label);
				// 批
				rss.setId(product.getShipId());
				ship = rssDao.getRssById(rss).getName();
				label = new Label(6, i, ship);
				sheet.addCell(label);
				// 入庫lot
				label = new Label(7, i, product.getInLot());
				sheet.addCell(label);
				// 出庫日期
				label = new Label(8, i, sdf.format(product.getOutputDate()));
				sheet.addCell(label);
				// 入庫日期
				label = new Label(9, i, sdf.format(product.getInputDate()));
				sheet.addCell(label);
				i++;
			}
	...
	private void updateActionPerformed(ActionEvent evt) {
		...
		while (iterator.hasNext()) {
			...
			model.addRow(((Product) entry.getValue()).getDescription(),
					...
					((Product) entry.getValue()).getQty(), repo, stor, ship,
					((Product) entry.getValue()).getInLot(),
					sdf.format(((Product) entry.getValue()).getOutputDate()),
					sdf.format(((Product) entry.getValue()).getInputDate()));
		}
	...
	class ProductTableModel extends AbstractTableModel {
		...
		public String[] colNames = { "Description", "WD", "IV", "Q'ty",
				...
				locale.getProperty("lot"), locale.getProperty("outputDate"),
		...
		public void addRow(String description, String wd, String iv, int qty,
				String repository, String stor, String ship, String lot,
				String outputDate, String inputDate) {
